...
export default {
  ...
  methods: {
    generatePrizes () {
      // สุ่มรางวัลที่ 1
      this.prizes.first = this.randomNumber(1000).toString().padStart(3, '0')

      // สุ่มรางวัลที่ 2
      this.prizes.second = this.generateUniqueNumbers(1000, 3).map(
        (num) => num
      )

      // สุ่มเลขข้างเคียงรางวัลที่ 1
      this.prizes.nearby = [parseInt(this.prizes.first, 10) - 1, parseInt(this.prizes.first, 10) + 1].map(
        (num) => {
          if (num < 0) {
            return 900 // กรณีที่น้อยกว่า 0
          } else if (num > 999) {
            return 0 // กรณีที่มากกว่า 999
          } else {
            return num
          }
        }
      ).map(num => num.toString().padStart(3, '0')) // เติมศูนย์หน้าให้ครบ 3 หลัก
    ...
    checkPrize () {
      ...
      if (number === this.prizes.first.toString().padStart(3, '0')) {
        messages.push('คุณถูกรางวัลที่ 1!')
      }
      // ตรวจสอบรางวัลที่ 2
      if (this.prizes.second.map(num => num.toString().padStart(3, '0')).includes(number)) {
        messages.push('คุณถูกรางวัลที่ 2!')
      }
      // ตรวจสอบเลขข้างเคียงรางวัลที่ 1
      if (this.prizes.nearby.map(num => num.toString().padStart(3, '0')).includes(number)) {
        messages.push('คุณถูกรางวัลเลขข้างเคียงรางวัลที่ 1!')
      }
      // ตรวจสอบเลขท้าย 2 ตัว
      if (this.prizes.lastTwoDigits.includes(this.userNumber.slice(-2).padStart(2, '0'))) {
        messages.push('คุณถูกรางวัลเลขท้าย 2 ตัว!')
      }
      // แสดงผลลัพธ์
      if (messages.length > 0) {
        this.resultMessage = messages.join(' และ ')
      } else {
        this.resultMessage = 'คุณไม่ได้ถูกรางวัลใด ๆ'
      }
    }
  }
}
